...
INPUT_SIZE    = 30      # 2 bits per timestep
RNN_HIDDEN    = 30
OUTPUT_SIZE   = 30      # 1 bit per timestep
...
'''
weights = {
    'out': tf.Variable(tf.random_normal([None, None, OUTPUT_SIZE]))
}
biases = {
    'out': tf.Variable(tf.random_normal([OUTPUT_SIZE]))
}

predicted_outputs = tf.matmul(predicted_outputs, weights['out']) + biases['out']
'''
...
#accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(predicted_outputs,1), tf.argmax(outputs,1)), tf.float32))
...
for epoch in range(45):
    ...
    for _ in range(ITERATIONS_PER_EPOCH):
        ...
        '''
        epoch_loss += session.run([loss, train_fn], {
            inputs: x,
            outputs: y,
        })[0]
